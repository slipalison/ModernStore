...
namespace ModernStore.API.Controllers
{
    public class AccountController : BaseController
    {
        ...
        public AccountController(IOptions<TokenOptions> jwtOptions, ICustomerRepository repository, IUow uow) : base(uow)
        {
        ...
        [HttpPost, AllowAnonymous, Route("v1/authenticate")]
        public async Task<IActionResult> Post([FromForm] AuthenticateUserCommand command)
        {
            ...
            var identity = await GetCalims(command);
        ...
        private Task<ClaimsIdentity> GetCalims(AuthenticateUserCommand command)
        {
            var custumer = _repository.GetByUsername(command.Username);
            if (custumer == null)
                return Task.FromResult<ClaimsIdentity>(null);

            if (!custumer.User.Authenticate(command.Username, command.Password))
                return Task.FromResult<ClaimsIdentity>(null);

            _customer = custumer;

            return Task.FromResult(new ClaimsIdentity(
                new GenericIdentity(custumer.User.Username, "Token"),
                new[]
                {
